...
const TherapistMaterialDetailsScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      if (!user) {return;}
      console.log(`[TherapistMaterialDetailsScreen] GET /materials/${id}/details`);
      try {
        const data = await getMaterialDetails(user.token, id);
        console.log('[TherapistMaterialDetailsScreen] Materiał pobrany:', data);
        setMaterial(data);
      } catch (error) {
        console.error('[TherapistMaterialDetailsScreen] Error fetching material details:', error);
      } finally {
        setLoading(false);
      }
    };
          ...
          <View style={{ flex: 1 }}>
            ...
            {therapist && therapist.name ? (
              <View style={styles.therapistHeaderContainer}>
                ...
                <Text style={styles.therapistHeaderText}>{therapist.name}</Text>
              </View>
            ) : null}
          </View>
        </View>
...
export default TherapistMaterialDetailsScreen;
